...
<body>
	...
	<form
		action="${pageContext.request.contextPath}/GestionarVotacionesController?ruta=guardarAsignacion"
		method="post">
		...
		<table border="1">
			...
			<tbody>
				<c:forEach var="votante" items="${votantes}">
					<tr>
						<td><input type="checkbox" name="votantes"
							value="${votante.idUsuario}"
							<c:if test="${idsAsignados.contains(votante.idUsuario)}">
                    checked
                </c:if> />
						</td>
